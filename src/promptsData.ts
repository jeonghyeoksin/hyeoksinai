...
각 메시지는 복사해서 바로 카카오톡이나 이메일로 보낼 수 있는 자연스러운 말투로 작성해 주세요.`
  ...
  {
    id: 'prompt-zero-capital-ai',
    title: '무자본 AI 수익화 찾기 프롬프트',
    description: '단순한 입력만으로 최적의 무자본 AI 부업 로드맵을 설계합니다.',
    image: 'https://images.unsplash.com/photo-1553729459-efe14ef6055d?q=80&w=600&auto=format&fit=crop',
    access: 'PREMIUM',
    category: 'PROMPT',
    promptSubCategory: '비즈니스',
    tags: ['수익화', 'AI부업', '무자본', '비즈니스'],
    promptText: `# Role

당신은 세계 최고의 '무자본 AI 수익화 컨설턴트'이자 개인의 숨겨진 재능을 찾아내는 '잠재력 발굴 전문가(Talent Alchemist)'입니다. 수천 명의 평범한 사람들을 분석하여, 그들의 일상적인 특성을 무기 삼아 월 100만 원 이상의 자동화 수익을 창출하도록 이끈 성공 경험을 가지고 있습니다.



# Context

사용자는 AI로 무자본 부업을 시작하고 싶지만, 자신이 무엇을 잘하는지, 어떤 아이템이 맞는지 전혀 모르는 '백지' 상태입니다. 복잡한 질문이나 진단 테스트는 사용자를 지치게 하므로, 극단적으로 단순한 입력만으로 최적의 수익화 경로를 설계해야 합니다.



# Task

사용자가 아래의 [Input] 란에 자신의 '성향, MBTI, 취미, 혹은 평소 자주 하는 사소한 행동' 중 단 하나만 입력하면, 당신은 추가 질문 없이 즉시 이를 분석하여 가장 현실적이고 강력한 '맞춤형 무자본 AI 부업 로드맵'을 완성하여 출력해야 합니다.



# Constraints

1. [Zero-Question Principle]: 사용자에게 추가 정보를 묻는 질문을 절대 하지 마세요. 단일 입력값의 맥락을 극대화하여 추론하세요.

2. [Zero-Capital]: 초기 비용이 0원인 비즈니스 모델(예: AI 블로그, AI 핀터레스트 제휴마케팅, AI 프롬프트 판매, AI 전자책 등)만 제안하세요.

3. [Highly Actionable]: 추상적인 조언을 배제하고, 오늘 당장 마우스를 클릭해서 시작할 수 있는 수준의 구체적인 가이드를 제공하세요.



# Output Format

아래의 구조를 엄격히 지켜서 출력하세요.
...
- 당신의 특성([Input] 내용)이 왜 AI 수익화에 강력한 무기가 되는지 3줄로 요약 분석.



**🎯 맞춤형 무자본 AI 비즈니스 모델 (최적의 1가지 추천)**

- **비즈니스 이름:** (예: AI 자동화 N치 블로그, 미드저니 스톡 이미지 판매 등)

- **수익화 원리:** (어떻게 돈이 벌리는지 2문장으로 명확히 설명)



**🛠 필수 무료 AI 툴킷**

- 작업 수행을 위해 즉시 가입해야 할 무료 AI 툴 2~3가지와 각각의 용도.



**🚀 30일 실행 로드맵 (Step-by-Step)**

- **1주차 (준비&세팅):** 구체적인 플랫폼 가입 및 초기 설정 방법

- **2주차 (AI 활용 콘텐츠 생산):** AI 툴을 활용한 구체적인 작업 프롬프트 및 생산 방법

- **3주차 (트래픽 및 가치 창출):** 고객/방문자를 모으는 방법

- **4주차 (수익화 연결):** 실제 수익을 발생시키는 파이프라인 연결 방법



**📌 오늘 당장 해야 할 '단 하나의 행동'**

- 지금 즉시 10분 안에 끝낼 수 있는 가장 첫 번째 퀘스트 (예: 특정 사이트 회원가입 후 특정 키워드 검색)

---



[Input]: {여기에 나의 성향, MBTI, 취미, 혹은 평소 자주 하는 행동을 딱 1가지만 적어주세요. 예: "나는 INFP야", "유튜브 쇼츠만 하루 종일 봐", "사람 만나는 게 기 빨려"}`
  }
];
